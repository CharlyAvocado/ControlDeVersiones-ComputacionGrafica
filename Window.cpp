...
int Window::Initialise()
{
	...
	//CREAR VENTANA
	mainWindow = glfwCreateWindow(width, height, "Practica 09.2: Animacion Avanzada", NULL, NULL);
...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	{
		theWindow->fuego = !theWindow->fuego;
	}
	if (key == GLFW_KEY_2 && action == GLFW_PRESS)
	{
		theWindow->hielo = !theWindow->hielo;
	}
	if (key == GLFW_KEY_3 && action == GLFW_PRESS)
	{
		theWindow->arena = !theWindow->arena;
	}
	if (key == GLFW_KEY_4 && action == GLFW_PRESS)
	{
		theWindow->trueno = !theWindow->trueno;
	}
	if (key == GLFW_KEY_5 && action == GLFW_PRESS)
	{
		theWindow->gas = !theWindow->gas;
	}
	...
	if (key == GLFW_KEY_Q)
	{
		theWindow->arranque =  true;

	}
	if (key == GLFW_KEY_Q && action == GLFW_PRESS)
	{
		theWindow->arranque2 = true;

	}
	else {
		theWindow->arranque2 = false;
	}
	//Dado
	if (key == GLFW_KEY_UP && action == GLFW_PRESS)
	{
		theWindow->caida = true;
