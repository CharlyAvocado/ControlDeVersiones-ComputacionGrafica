...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Blackhawk
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(mainWindow.getTraslacion(), 5.0f, 6.0));

		glm::vec4 heliLocal = glm::vec4(0.0f, -0.225f, 0.0f, 1.0f); // Coordenadas de la luz
		glm::vec3 heliWorld = glm::vec3(model * glm::vec4(heliLocal));// Calcula la posición la luz 
		spotLights[2].SetPos(heliWorld);// Establece la posición de la luz
